...
class topK_MLP(nn.Module):
    def __init__(self, cfg: Dict):
        super(topK_MLP, self).__init__()
    ...
    def forward(self, x, y, l1_coeff=0.0):
        ...
        if self.k < self.d_hidden:
            threshold = torch.topk(activations, self.k, dim=1, largest=True, sorted=False).values[:, -1:]
            mask = activations >= threshold  # Retain only top-K activations
            activations = activations * mask  # Zero out other activations
